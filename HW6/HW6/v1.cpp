...
void randomMove(Robot & robot)
{
	...
	// for the rest 3 directions complete
...
int main()
{
	...
	player.SetName(nameM1);

	Robot monster2(0,7);
	monster2.SetColor(red);
	player.SetName(nameM2);

	Robot monster3(7,7);
	monster3.SetColor(red);
	player.SetName(nameM3);
	...
	else if (player.GetBagCount() > max(monster1.GetBagCount(), monster2.GetBagCount(), monster3.GetBagCount()))
		ShowMessage("Congratulations. " + nameP + " has won the game.");

	else
		ShowMessage(nameP + " LOST!!!");
